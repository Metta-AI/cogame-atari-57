...
when isMainModule:
  ...
  proc dial(attempts: int): WebSocket =
    ...
    for attempt in 0 ..< attempts:
      try:
        ...
        if attempt == 0:
          echo "atari-57 player: game not listening yet (", error.msg,
            "); retrying"
  ...
  if socket == nil:
    quit("atari-57 player: game never accepted a connection", 1)
  echo "atari-57 player: connected"
  ...
  # episode (paintball round 3, 2026-08-25 — inherited scar). The server now holds an
  ...
  while true:
    ...
    # lane for the whole episode and revives on reconnect, so a dropped socket
    ...
    if sessionFrames == 0 or reconnects >= ReconnectAttempts:
      break
    inc reconnects
    echo "atari-57 player: re-dialling the seat (attempt ", reconnects, ")"
