## The deterministic replay runtime shared by the native server and the wasm
## viewer. `coworld-ctf`'s `src/ctf/replay_runtime.nim`, byte-identical apart
## from the imports: `initReplayRuntime` / `advanceReplayFrame` /
## `buildReplayViewerPacket` are what make one module drive both hosts.
