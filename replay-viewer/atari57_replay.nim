## The static replay viewer's wasm entry.
##
## `coworld-ctf`'s `replay-viewer/atari57_replay.nim`, forked with the identifier
## rename only: the stage-note buffer, the `ABORTING_MALLOC` diagnostics, the
## `predictedViewerRenderBytes` / `WasmViewerBudgetBytes` capacity preflight
## and the `emscripten_exit_with_live_runtime` lifetime are all kept, because
## every one of them is scar tissue from a real hosted failure.
##
## It imports the SAME `src/lane/sim.nim` the native server ran, so every
## frame here is a genuine re-simulation of the recorded action bytes, hash-
## checked tick by tick.

import
  std/json,
  lane/[broadcast, global, replay_runtime, replays, sim]
...
## used to die with atari57_error_len() == 0 and no diagnostic at all). The
...
proc atari57LoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "atari57_load_replay", cdecl.} =
  try:
    lastError = ""
    ...
    runtimeLoaded = true
    let mapNote = " (board " & $MapWidth & "x" & $MapHeight & ")"
...
proc atari57Input(data: ptr uint8, length: cint)
    {.exportc: "atari57_input", cdecl.} =
  if runtimeLoaded:
    viewer.applyGlobalViewerMessage(data.bytesFromPointer(int(length)))

proc atari57Frame(): cint {.exportc: "atari57_frame", cdecl.} =
...
proc atari57PacketPointer(): ptr uint8
    {.exportc: "atari57_packet_ptr", cdecl.} =
  if packet.len == 0:
    nil
  else:
    packet[0].addr

proc atari57PacketLength(): cint {.exportc: "atari57_packet_len", cdecl.} =
  cint(packet.len)

proc atari57MismatchTick(): cint {.exportc: "atari57_mismatch_tick", cdecl.} =
  if runtimeLoaded:
    cint(replay.hashMismatchTick)
  else:
    -1

proc atari57ErrorPointer(): ptr uint8 {.exportc: "atari57_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc atari57ErrorLength(): cint {.exportc: "atari57_error_len", cdecl.} =
  cint(lastError.len)

proc atari57StagePointer(): ptr uint8 {.exportc: "atari57_stage_ptr", cdecl.} =
  ## The progress note (see stageNote above). Unlike atari57_error_*, this stays
...
proc atari57StageLength(): cint {.exportc: "atari57_stage_len", cdecl.} =
  cint(stageNoteLen)
...
when isMainModule and defined(emscripten):
  ...
  # module stays alive and JS keeps calling atari57_load_replay/atari57_frame. The
